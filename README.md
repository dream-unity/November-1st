...
Use Node 24.14+ within the Node 24 release line, or Node 26. The default host listens on port 3000. No provider credentials are required for keyless startup. See [deployment instructions](docs/DREAM_UNITY_DEPLOYMENT.md), [source provenance](UPSTREAM.md) and [host security and capabilities](host/README.md).
...
The visible **Live radio**, **CCTV cameras**, and **Traffic reports** buttons open searchable feeds with playback, source details and retry controls. They also work when WebGL is unavailable. Direct links: [radio](https://dream-unity.github.io/November-1st/?feed=radio), [cameras](https://dream-unity.github.io/November-1st/?feed=cctv), [traffic reports](https://dream-unity.github.io/November-1st/?feed=traffic). Radio is broadcaster audio. **CCTV opens in Live video**, with country filtering, official Caltrans streams and 89 owner-published international broadcasts across 55 countries and territories; **Snapshots** and **Clips / other videos** are separate filters. Selected live video starts muted, with Play/Pause/Retry controls. Provider-listed cameras may be offline; a failed stream is never silently replaced with a still. See the [global coverage and verification record](docs/GLOBAL_LIVE_CCTV.md) and [continuous-video repair record](docs/CCTV_LIVE_VIDEO_REPAIR.md). Official traffic reports currently cover Austin and Finland, with source timestamps and explicit stale/partial states. They are separate from the upstream simulated Street Traffic vehicles and optional TomTom speeds. **Show on globe** connects a selected feed to the full application when its globe is ready.

**Ukraine:** [live cameras](https://dream-unity.github.io/November-1st/?feed=cctv&country=UA) adds five official Bukovel tourism HLS streams with archive rejection and no snapshot fallback. [Ukraine radio](https://dream-unity.github.io/November-1st/?feed=radio&country=UA) uses a dedicated country directory and reviewed broadcaster fallback; stations without coordinates remain playable, with globe navigation disabled. This is not camera coverage of every city. See the [Ukraine coverage, source review and verification record](docs/UKRAINE_LIVE_MEDIA.md).
...
4. **Look through a public camera.** Turn on **CCTV** over Austin, London, California, or Finland. Direct camera feeds project _into_ the 3D city. International owner-published streams play in their official embedded player in the camera panel; they are not copied into globe textures. Cycle coverage to **VIEWSHED** and every camera draws its estimated coverage volume — where it reaches, and where it goes blind.
