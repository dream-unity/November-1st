...
test('hosted Ukraine radio reads the immutable source archive after cwd moves to writable state', async (t) => {
  const previousCwd = process.cwd();
  const root = await mkdtemp(path.join(os.tmpdir(), 'gev-ukraine-radio-'));
  ...
  await writeFile(path.join(sourceRoot, 'config/radio_sources.ukraine.json'), JSON.stringify([
    {
      id: 'test-public-broadcaster', name: 'Public Ukrainian radio',
      streamUrl: 'https://radio.example.org/live.mp3',
      sourcePage: 'https://example.org/listen', codec: 'MP3',
      playbackKind: 'live', verifiedAt: '2026-09-17T00:00:00Z',
      languages: ['Ukrainian'],
    },
  ]));
  ...
  const plugin = radioBrowserProxy({
    ...
    fetchImpl: async () => { throw new Error('directory temporarily offline'); },
  });
  ...
  const response = await fetch(`${origin}/api/radio/stations?country=UA&path=radio%2Fstations`);
  assert.equal(response.status, 200);
  assert.equal(response.headers.get('cache-control'), 'no-store');
  const result = await response.json();
  assert.equal(result.degraded, true);
  assert.equal(result.stations.length, 1);
  assert.equal(result.stations[0].sourceKind, 'curated-ukraine');
  assert.equal(result.stations[0].name, 'Public Ukrainian radio');
  assert.equal(result.stations[0].lat, null);
  assert.equal(result.stations[0].lon, null);
  const click = await fetch(`${origin}/api/radio/click/${result.stations[0].id}`, { method: 'POST' });
